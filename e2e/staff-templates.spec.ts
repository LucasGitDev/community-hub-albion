...
test("staff cria role, monta template com vagas e não consegue apagar role em uso (AC#1, AC#2, AC#3)", async ({ page }) => {
  ...
  const roleRow = catalog.getByRole("row").filter({ hasText: roleName });
  await expect(roleRow.getByText("sem template")).toBeVisible();
  await snap(page, `roles-${tag()}`);
  ...
  // AC#3: role em uso não pode ser apagada (visível na página de roles).
  await page.goto("/staff/roles");
...
test("staff exporta um template em YAML e importa de volta criando roles novas (TASK-038 AC#1, AC#2, AC#3, AC#4)", async ({ page }) => {
  ...
  // A role criada pela importação aparece em uso no catálogo (página própria, TASK-040).
  await page.goto("/staff/roles");
  await expect(page.getByRole("region", { name: "Catálogo de roles" }).getByRole("row").filter({ hasText: roleNova }).getByText("em 1 template")).toBeVisible();
...
test("membro sem permissão não abre a página de roles (TASK-040 AC#3)", async ({ page }) => {
  await login(page, "72000000000000007", "membroR");
  await page.goto("/staff/roles");
  await expect(page.getByRole("heading", { name: "Acesso negado" })).toBeVisible();
