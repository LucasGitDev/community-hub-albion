...
- Máquina disputada (cliente do jogo aberto, `uptime` com load alto): `E2E_WORKERS=2` no comando do gate — sem ela, specs sem relação com a mudança estouram os 30s e o gate reprova por ambiente.
...
- Mudança **só de visibilidade** (nenhum componente ou texto novo, só quem enxerga o que já existe) não exige as skills de design, mas **exige revisão visual por screenshot no papel afetado** (decisão N7).
- **Gate local como check do PR**: `pnpm ship:pr` roda o gate e publica o status `summary` no commit (é o check obrigatório do ruleset). Só publica com árvore limpa e commit já no remoto, e publica `failure` quando reprova. O Actions continua rodando no **push da main** (verificação em ambiente neutro depois do merge) e em **PR de fork**.
