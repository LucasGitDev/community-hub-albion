...
export class AppModule {
  static register(env: Env, options: AppModuleOptions = { bot: true }): DynamicModule {
    return {
      module: AppModule,
      imports: [DbModule.register(env.DATABASE_URL), AuthModule.register(env), NickModule.register(env), MembersModule.register(env), TemplatesModule.register(env), EventsModule.register(env), ...(options.bot ? [BotModule.register(env)] : [])],
